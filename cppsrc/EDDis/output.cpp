...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		{
			int xx_42_xx = 2;
			MPI_Bcast(&xx_42_xx, 1, MPI_INT, 0, MPI_COMM_WORLD);
		...
		{
			int xx_45_xx = 3;
			MPI_Bcast(&xx_45_xx, 1, MPI_INT, 0, MPI_COMM_WORLD);
			int xx_29_xx = 0;
			int xx_30_xx = K - 1;
		...
		{
			for (int xx_46_xx = 0; xx_46_xx <= K - 1; xx_46_xx++)
			{
				xx_28_xx.AddEle(xx_46_xx);
			}
		}
		...
		while (error > EPS && iter <= maxIter)
		{
			...
			{
				int xx_47_xx = 4;
				MPI_Bcast(&xx_47_xx, 1, MPI_INT, 0, MPI_COMM_WORLD);
				clusList.distributeList(numprocs);
			...
			{
				int xx_49_xx = 5;
				MPI_Bcast(&xx_49_xx, 1, MPI_INT, 0, MPI_COMM_WORLD);
				...
				for (int xx_53_xx = 1; xx_53_xx < numprocs; xx_53_xx++)
				{
					xx_50_xx = xx_50_xx.deserializeRecv(xx_53_xx);
					for (auto iterator = xx_50_xx.htmap.begin(); iterator != xx_50_xx.htmap.end(); iterator++)
					{
						uint i = iterator->first;
						clusList[i].wsum += xx_50_xx.htmap[i];
					}
				}
				for (int xx_54_xx = 1; xx_54_xx < numprocs; xx_54_xx++)
				{
					xx_51_xx = xx_51_xx.deserializeRecv(xx_54_xx);
					for (auto iterator = xx_51_xx.htmap.begin(); iterator != xx_51_xx.htmap.end(); iterator++)
					{
						uint i = iterator->first;
						clusList[i].musum += xx_51_xx.htmap[i];
					}
				}
				for (int xx_55_xx = 1; xx_55_xx < numprocs; xx_55_xx++)
				{
					xx_52_xx = xx_52_xx.deserializeRecv(xx_55_xx);
					for (auto iterator = xx_52_xx.htmap.begin(); iterator != xx_52_xx.htmap.end(); iterator++)
					{
						uint i = iterator->first;
						clusList[i].sigmasum += xx_52_xx.htmap[i];
					}
				}
			}
			else
		...
		{
			int xx_65_xx = 7;
			MPI_Bcast(&xx_65_xx, 1, MPI_INT, 0, MPI_COMM_WORLD);
			clusList.broadcastList(numprocs);
	...
	int xx_67_xx = -1;
	MPI_Bcast(&xx_67_xx, 1, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Finalize();
