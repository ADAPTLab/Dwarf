...
double GMM(Point x, Point mu, Point sigma, double prob) {
	try {
		...
		for (auto &i : xx_0_xx.Elements()) {
			sum = sum
					+ ((x.GetEleAtIndex(i) - mu.GetEleAtIndex(i))
							* (x.GetEleAtIndex(i) - mu.GetEleAtIndex(i))
							/ sigma.GetEleAtIndex(i));
		...
		std::cout
				<< std::string(e.what()) + " At line no. 25, function "
						+ "GMM in file //home\/master\/git\/dwarf\/CompilerDev\/Driver\/em9thOct17.dw.preprocessed"
				<< std::endl;
	}
}
double convergence(List<EMCluster> clusList) {
	try {
		...
		std::cout
				<< std::string(e.what()) + " At line no. 41, function "
						+ "convergence in file //home\/master\/git\/dwarf\/CompilerDev\/Driver\/em9thOct17.dw.preprocessed"
				<< std::endl;
	}
}
double distanceEuclideanSquared(Point A, Point B) {
	try {
		...
		std::cout
				<< std::string(e.what()) + " At line no. 46, function "
						+ "distanceEuclideanSquared in file //home\/master\/git\/dwarf\/CompilerDev\/Driver\/em9thOct17.dw.preprocessed"
				<< std::endl;
	}
}
Point pointSquare(Point A) {
	try {
		...
		std::cout
				<< std::string(e.what()) + " At line no. 52, function "
						+ "pointSquare in file //home\/master\/git\/dwarf\/CompilerDev\/Driver\/em9thOct17.dw.preprocessed"
				<< std::endl;
	}
}
