...
/*---< mpi_read() >----------------------------------------------------------*/
double** mpi_read(int       isBinaryFile,  /* flag: 0 or 1 */
                 ...
                 MPI_Comm  comm)
{
    ...
    if (isBinaryFile) {  /* using MPI-IO to read file concurrently */
        ...
        assert(objects != NULL);
        objects[0] = (double*) malloc((*numObjs)*(*numCoords) * sizeof(double));
        ...
        if (rank == 0) {
            ...
            for (i=1; i<nproc; i++) {
                ...
                MPI_Send(objects[index], msg_size*(*numCoords), MPI_DOUBLE,
                         i, i, comm);
                index += msg_size;
            }

            /* reduce the objects[] to local size */
            objects[0] = (double*) realloc(objects[0],
                                 (*numObjs)*(*numCoords)*sizeof(double));
            assert(objects[0] != NULL);
            objects    = (double **) realloc(objects, (*numObjs)*sizeof(double*));
            assert(objects != NULL);
        }
        else {
            /*  local numObjs */
            ...
            MPI_Recv(objects[0], (*numObjs)*(*numCoords), MPI_DOUBLE, 0,
                     rank, comm, &status);
        }
    }
              ...
              MPI_Comm   comm)
{
    ...
    char       outFileName[1024], fs_type[1024], str[1024], *delim;
    MPI_File   fh;
    MPI_Status status;
    ...
    if (rank == 0)
	{
        ...
        if (isOutFileBinary)
		{
            ...
            MPI_File_write(fh, clusters[0], numClusters*numCoords, MPI_DOUBLE, &status);
        }
        else
		{
    ...
    if (rank == 0) printf("\t\t\tWriting membership of N=%d data objects to file \"%s.membership\"\n",    totalNumObjs, delim);
	...
	{
        if (rank == 0)
		{ /* gather membership[] from all processes ----------*/
			...
			{
                sprintf(str, "%d %d\n", j, membership[j]);
                err = MPI_File_write(fh, str, strlen(str), MPI_CHAR, &status);
				if (err != MPI_SUCCESS)
				{
		            char errstr[MPI_MAX_ERROR_STRING];
		            int  errlen;
            ...
            for (i=1; i<nproc; i++)
			{
                ...
                err = MPI_Recv(membership, numObjs, MPI_INT, i, i, comm, &status);
				if (err != MPI_SUCCESS)
				{
					char errstr[MPI_MAX_ERROR_STRING];
					int  errlen;
