...
void __ff__6()
{
	...
	int tcount = 4;
	double *t_wsum[tcount];
	Point *t_musum[tcount];
	Point *t_sigmasum[tcount];

	for (int t = 0; t < tcount; t++)
	{
		t_wsum[t] = new double[K];
		t_musum[t] = new Point[K];
		t_sigmasum[t] = new Point[K];
		for (int i = 0; i < K; i++)
		{
			t_wsum[t][i] = 0.0;
			t_musum[t][i].NumEles = MaxEles;
			t_sigmasum[t][i].NumEles = MaxEles;

			t_musum[t][i].InitializePoint();
			t_sigmasum[t][i].InitializePoint();
		}
	}
...
#pragma omp ordered
		{
			for (int i = 0; i < K; i++)
			{
				clusList[i].wsum += t_wsum[tid][i];
				clusList[i].musum += t_musum[tid][i];
				clusList[i].sigmasum += t_sigmasum[tid][i];
			}
		}
	}
